 ...
 * NOTE: Pattern should be in double quotes
...
int main(int argc,char *argv[]) {
    ...
    if (argi>=argc) {
        ...
        fprintf(stderr,"  Tip: Always quote PATTERN to avoid shell misinterpretation.\n");
        return 1;
    }
    ...
    if(argi<argc) {
        ...
        if(!fp) {
            fprintf(stderr,"Error: custom_grep cannot open '%s'\n",argv[argi]);
            return 1;
        }
        argi++; // advance past the filename 

        // Warn about extra arguments that will be ignored
        if (argi<argc) {
            fprintf(stderr,"custom_grep: warning: extra arguments ignored:");
            while (argi<argc)
                fprintf(stderr, " '%s'",argv[argi++]);
            fprintf(stderr,"\n");
        }
    } else {
        // No file given — reading from standard input
        fprintf(stderr, "custom_grep: no file specified, reading from stdin (Ctrl+D to end)\n");
    }
